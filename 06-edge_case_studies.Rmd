...
las_cs2 <- readLAS("data/06/02/ctg/1kmZ157050546602020L.laz")
...
road_buffer <- st_read("data/06/02/road_buffer.shp")
chm_cs2 <- raster("data/06/02/chm.tif")
chm_cs2_clip <- raster("data/06/02/chm_clip.tif")
conduct_cs2 <- raster("data/06/02/conductivity.tif")
...
las_cs3 <- readLAS("data/06/03/ctg/1kmZ174660536302018L.laz")
dtm_cs3 <- raster("data/06/03/DTM.tif")
road_cs3 <- st_read("data/06/03/road.shp")
updated_road_cs3 <- st_read("data/06/03/updated_road.shp") #no water

#Case study 4: Narrow and works well
las_cs4 <- readLAS("data/06/04/ctg/1kmZ156480546602020L.laz")
las_infolder <- ("data/06/04/ctg/")
ctg_cs4 <- readLAScatalog(las_infolder, filter = "-drop_withheld -keep_random_fraction 0.25")
dtm_cs4 <- raster("data/06/04/DTM.tif")
road_cs4 <- st_read("data/06/04/road.shp")
upd_road_cs4 <- st_read("data/06/04/road_upd.shp")
```
...
## Case Study 4: Narrow Road
### Overview: 

The road extraction method developed by [Roussel et al. (2022)](https://www.sciencedirect.com/science/article/pii/S1569843222002084#d1e1483) uses is suitable for locating narrow roads. The method parameters set the smallest drivable road width at 1 m. In this case study, we demonstrate the effectiveness of the `measure_roads()` function ad relocating a narrow road with a drivable road width of 1.2 m. Users can adjust the standard parameters to specify the minimum road width. For example, if a user applies the method in an area where roads must be over 2 m (for a specific vehicle), they can set the minimum road width to 2 m. 

In this case study, we update a road in the Dog River-Matawin Forest in Ontario. We do not change the standard parameters in this case study, as they are sufficient for updating narrow roads. The road is located in a relatively flat area but can be accurately detected, as the plot below demonstrates:

```{r, disp = FALSE}
las_clip <- clip_circle(las_cs4, 648350, 5466120, 50)
dtm <- rasterize_terrain(las_clip, res = 0.2, algorithm = tin())
dtm <- raster(dtm)
plot(dtm)
plot(road_cs4, add = TRUE)
```
### Data, Method, and Application: {#sec-method-and-application}
...
The updated road is relocated accurately, and the vector information includes information about the state and width of the road, as well as information on road curvature, shoulders, and canopy cover.

```{r, echo = FALSE, fig.width = 7.1, fig.height = 7}
road_poly <- sf::st_buffer(road_cs4, road_cs4$ROADWID/2)

url <- "https://servicesmatriciels.mern.gouv.qc.ca:443/erdas-iws/ogc/wmts/Inventaire_Ecoforestier/Inventaire_Ecoforestier/default/GoogleMapsCompatibleExt2:epsg:3/{z}/{y}/{x}.jpg"
m <- mapview::mapview(list(road_cs4, road_poly),
...
## Case study 5: road covered by deciduous trees
