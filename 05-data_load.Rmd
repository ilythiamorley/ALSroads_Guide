...
`ALSroads` users may be provided a 1 m resolution DTM that covers the LAS catalog coverage. If DTM data is provided, then users can load this information using the `raster` package.
...
### Generating DTM data using `grid_terrain()` {#sec-generating-dtm-data-using-grid_terrain}
...
The `grid_terrain()` function interpolates ground points and creates a rasterized DTM. The interpolation can be done using 3 methods: "knnidw", "delaunay", or "kriging" (for detailed information on each method, refer to the [lidR book](https://r-lidar.github.io/lidRbook/io.html#asprs-compliance)).
...
To generate a DTM model with the TIN algorithm we the `grid_terrain()` function and specify the algorithm using `algorithm = tin()`.

``` r
dtm_tin <- grid_terrain(ctg, res = 1, algorithm = tin())
```
