...
2. spatially indexed: to query areas of interest quickly an improve performamce
...
5. have a density of 5-10 points/m²:  less than that is not guaranteed to work, more than that is not useful is will only increases the computation time. If more than that the point cloud can be decimated on-the-fly using the `filter` argument (see lidR book [chapter 2.1.1](https://r-lidar.github.io/lidRbook/io.html#filter))
...
```{r, fig.width=5}
library(sf)
roads <- st_read("data/05_data_load/roads_las.shp", quiet = TRUE)
plot(ctg)
...
1. It helps to find the road properly by masking the lakes that can easily be interpreted as highly drivable areas. Indeed lakes are perfectly flat, homogeneous and cleared areas. On a geometrical point-of-view this is the best place to drive a car and, unless the lidar point cloud is water classified there is no way to guess that these nice flat, cleared areas are not passable.
2. Water bodies are required to get correct results on roads that cross rivers using a bridge. In a DTM a river is only a gap with strong slopes each edges. This is therefore an impassable obstacle. The bridge is visible in the point cloud but in absence of point classified as bridge, bridge points have the same status than another point. Geometrically speaking a bridge in the point cloud is just a set of points above the grounds, like trees actually. Thus a not labelled bridge above a deep is technically similar to a tree above the ground.

Providing a map of water bodies enables to inform that a road is known to cross a river and to take that into account to allow the passage of a gap that should be otherwise considered impassable.