...
In the following figure, the red line is the road existing road segments recorded and maintained by an authority. This line is not associated with any information about the road state, and the road path is inaccurate and does not follow the actual road. The method uses this line as prior information to relocate the true road path and derive information about the characteristics of the road. In the example below, the algorithm found that the road is of class 1 (good state) and has a width of 9 m.

![](images/screenshot.png)

The `ALSroads` package is open source and integrated within the geospatial R ecosystem (i.e., `raster` and `sf`). The package utilizes the `lidR` package, developed for manipulating and visualizing ALS data. This user guide has been written to support users with varying levels of experience working with LiDAR data.
