...
`ALSroad` is an R package and thus requires an installation of the R programming language that can be downloaded for [Windows](https://cran.r-project.org/bin/windows/base/), [macOS](https://cran.r-project.org/bin/macosx/), and [Linux](https://cran.r-project.org/).
...
## Installing `ALSroad` {#sec-installing-alsroad}

`ALSroads` is available on [Github](https://github.com/Jean-Romain/ALSroads) but is not part of the available packages on CRAN. To install it you must first install the package `remotes` then use `remotes` to install the package from Github seamlessly.
