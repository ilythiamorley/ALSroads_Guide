...
As detailed in section \@ref(sec-road-extraction-data-requirements), the required input data for updating the existing road network are:
...
1. Ensure the coordinate reference system of the existing roads network and `LAScatalog` match.
...
In this example, we do not include water bodies, refer to section \@ref(sec-waterbodies) for an example application with water bodies. Similarly, here we do not adjust the `ALSroads` default parameters; refer to section \@ref(sec-edge-case-studies) for example, applications of the method with custom parameters.
...
```{r plot-roads-network, warning = FALSE, fig.align='center', fig.width=6, fig.height=4}
...
In addition, following road extraction, the new road width attribute can be used to buffer the road and show the road footprint. Buffering is performed using the `sf` package function `st_buffer()`.

``` r
road_poly <- sf::st_buffer(updated_roads, updated_roads$ROADWIDTH/2)
```

```{r plot-buff-rd, warning = FALSE, fig.align='center', fig.width=6, fig.height=4}
url <- "https://servicesmatriciels.mern.gouv.qc.ca:443/erdas-iws/ogc/wmts/Inventaire_Ecoforestier/Inventaire_Ecoforestier/default/GoogleMapsCompatibleExt2:epsg:3/{z}/{y}/{x}.jpg"
...
```
<center> ![](images/road_class_plot.png) <center>
...
```
<center> ![](images/road_width_plot.png) <center>