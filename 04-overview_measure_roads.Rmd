...
1.  Insure the coordinate reference system of the existing roads network and `LAScatalog` match.
...
-   PERCABOVEROAD - The percentage of point above the road in a range [0.5, 5] m
...
In this example, we do not include water bodies, refer to section \@ref(sec-waterbodie) for an example application with water bodies. Similarly, here we do not adjust the `ALSroads` default parameters, refer to section \@ref(sec-edge-case-studies) for example applications of the method with custom parameters.
...
```{r plot-roads-network, warning = FALSE, fig.align='center', fig.width=8, fig.height=6}
...
```{r plot-buff-rd, warning = FALSE, fig.align='center', fig.width=8, fig.height=6}
url <- "https://servicesmatriciels.mern.gouv.qc.ca:443/erdas-iws/ogc/wmts/Inventaire_Ecoforestier/Inventaire_Ecoforestier/default/GoogleMapsCompatibleExt2:epsg:3/{z}/{y}/{x}.jpg"
