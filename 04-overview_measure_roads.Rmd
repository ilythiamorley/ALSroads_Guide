...
roads_upd <-st_read("E:/data/05_data_load/updated_roads.shp")

roads1 <- roads %>%
dplyr::filter(FNODE_ %in% c(8041))

roads_upd1 <- roads_upd %>%
dplyr::filter(FNODE_ %in% c(8041))

road_poly <- sf::st_buffer(roads_upd1, roads_upd1$ROADWID/2)
...
m = mapview::mapview(list(roads_upd1, road_poly),
...
```{r}
ggplot(data=roads_upd, aes(x=CLASS, y= ROADWID, fill=CLASS)) +
