...
las_infolder <- ("data/08_edge_cs1/las/")
ctg_cs1 <- readLAScatalog(las_infolder, filter = "-drop_withheld -keep_random_fraction 0.25")
dtm_cs1 <- raster("data/08_edge_cs1/cs_1_DTM.bil")
rds_cs1 <- st_read("data/08_edge_cs1/cs_1_roads.shp")
upd_rds_cs1 <- st_read("data/08_edge_cs1/cs_1_upd_roads.shp") # Standard Parameters
```
...
```{r plot-dtm-rayshaded, rgl = TRUE, warning  = FALSE, fig.height=15, eval = FALSE, include= FALSE}
library(rayshader)
...
![](docs/images/DTM/dtm_ctg_rayshader.PNG)
...
```{r plot_dc_roads, echo = FALSE, rgl = TRUE, fig.width = 4, fig.height = 3}
m = mapview::mapview(list(dc_roads, dc_updated_roads),
...
slope <- terrain(dtm, opt=c('slope'), unit='radians')
aspect <- terrain(dtm, opt=c('aspect'), unit='radians')
hill = hillShade(slope, aspect, 40, 270)
plot(hill, col =gray(0:30/30), legend = FALSE)
```
...
We adjust the the standard `measure_roads()` parameters, explained in section \@ref().
