...
The `rasterize_terrain()` function interpolates ground points and creates a DTM. Here we demonstrate the Triangular irregular network (tin) interpolation method. The TIN method is fast, efficient, and generates good DTMs. To generate a DTM model with the TIN algorithm we the `rasterize_terrain()` function and specify the algorithm using `algorithm = tin()`.
