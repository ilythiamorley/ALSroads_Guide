...
The `print()` function can be used to summarize information about the catalog, such as the number of points and density of points.
...
`ALSroads` users may be provided with a 1 m resolution DTM that covers the `LAScatalog` coverage (refer to section \@ref(sec-dtm-data) for DTM requirements). If DTM data is provided, users can load this information using the `raster` package.
