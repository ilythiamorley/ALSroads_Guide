...
A `LAScatalog` is a representation in R of a LAS file or a collection of LAS files not loaded in memory. In `lidR`, the function `readLAScatalog()` creates an object that represents, in R, a collection of LAS files not loaded in memory.

``` r
ctg <- readLAScatalog("path/to/ctg/files")
```

The `print()` function can be used to summaries information about the catalog, such as the number of points, and density of points.
...
`ALSroads` users may be provided a 1 m resolution DTM that covers the `LAScatalog` coverage (refer to section \@ref(sec-dtm-data) for DTM requirements). If DTM data is provided, then users can load this information using the `raster` package.
...
```{r, message=FALSE, fig.align='center', fig.width=8, fig.height=6}
